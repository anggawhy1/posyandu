...
<div class="container mx-auto px-6 py-6">
    ...
    <div class="bg-white shadow-lg rounded-lg p-6">
        <h2 class="text-2xl font-semibold text-gray-700 mb-4">Form Data Remaja Putri</h2>
        <form action="<?= base_url('/data-remaja/store') ?>" method="post" class="grid grid-cols-1 md:grid-cols-2 gap-6" onsubmit="return submitForm(event)">
            ...
            <div>
                ...
                <input type="text" name="nama_lengkap" class="w-full border border-gray-300 rounded-lg p-2 mt-1" required>
            </div>

            <div>
                <label class="block text-gray-700 font-medium">NIK <span class="text-red-500">*</span></label>
                <input type="text" name="nik" class="w-full border border-gray-300 rounded-lg p-2 mt-1" required pattern="\d{16}">
            </div>

            <div>
                <label class="block text-gray-700 font-medium">Tanggal Lahir <span class="text-red-500">*</span></label>
                <input type="date" name="tanggal_lahir" class="w-full border border-gray-300 rounded-lg p-2 mt-1" required>
            </div>

            <div>
                <label class="block text-gray-700 font-medium">Golongan Darah <span class="text-red-500">*</span></label>
                <select name="golongan_darah" class="w-full border border-gray-300 rounded-lg p-2 mt-1" required>
            ...
            <div>
                <label class="block text-gray-700 font-medium">Kadar HB (g/dL) (Opsional)</label>
            ...
            <div>
                ...
                <select name="alamat" class="w-full border border-gray-300 rounded-lg p-2 mt-1" required>
            ...
            <div>
                <label class="block text-gray-700 font-medium">Nomor Telepon (Opsional)</label>
                <input type="text" name="nomor_telepon" class="w-full border border-gray-300 rounded-lg p-2 mt-1">
            </div>
...
<script>
    function submitForm(event) {
        ...
        .then(data => {
            if (data.success) {
                document.getElementById("successModal").classList.remove("hidden"); // Menampilkan modal sukses
                setTimeout(() => { 
                    closeModal();
                    window.location.reload(); // Reload halaman setelah modal muncul
                }, 2000);
            }
        }).catch(error => {
